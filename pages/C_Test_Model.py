...
def train_test_split(X, y, test_size=0.3, random_state=None):
    ...
    - test_size: float, default=0.3, proportion of the dataset to include in the test split
    ...
    - X_test: pandas DataFrame or numpy array, testing features
    ...
    num_instances = len(X)
    num_test = int(test_size * num_instances)
    ...
    X_test = X.iloc[test_indices] if isinstance(
        X, pd.DataFrame) else X[test_indices]
    y_train = y.iloc[train_indices] if isinstance(
        y, pd.Series) else y[train_indices]
    y_test = y.iloc[test_indices] if isinstance(
        y, pd.Series) else y[test_indices]
...
def plot_roc_curve(X_train, X_val, y_train, y_val, trained_models, model_names):
    ...
    for i, trained_model in enumerate(trained_models):
        ...
        for threshold in threshold_values:
            ...
            train_precision, train_recall = compute_precison_recall(
                train_predictions, y_train.to_numpy())
            ...
            val_precision, val_recall = compute_precison_recall(
                val_predictions, y_val.to_numpy())
...
def restore_data_splits(df):
    ...
    if (X_train is None):
        ...
        X_train, X_val, y_train, y_val = train_test_split(
            X, Y, test_size=number/100, random_state=42)
...
###################### FETCH DATASET #######################
df = None
# df = fetch_dataset() # Commented out for demonstration purpose
